...
| Switch: Buzzer | Optional buzzer on/off (`enableBuzzerControl`, JSON config only). MIOT models use a boolean buzzer flag; legacy models receive a volume value (100/0). **Not available on `zhimi.airpurifier.pro`** — force-disabled at runtime. To enable on supported models, add `"enableBuzzerControl": true` to your JSON config. |
...
| `enableBuzzerControl` | boolean | No | Expose Buzzer on/off switch service (default `false`, JSON config only). **Not available on `zhimi.airpurifier.pro`** — force-disabled at runtime. A warning is logged if explicitly set for the pro model. |
