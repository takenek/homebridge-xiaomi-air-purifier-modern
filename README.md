...
| Switch: Buzzer | Optional buzzer on/off (`enableBuzzerControl`) for supported models. Not exposed for `zhimi.airpurifier.pro` due to firmware incompatibility. MIOT models use a boolean buzzer flag; legacy models receive a volume value (100/0). |
...
| `enableBuzzerControl` | boolean | No | Expose Buzzer on/off switch service (default `false`, ignored for `zhimi.airpurifier.pro`) |
