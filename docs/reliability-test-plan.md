...
10. **Mute alias normalization**: Pro-state verification normalizes `mute` alias values that arrive as strings (`"1"`/`"0"`), so `"1"` is correctly interpreted as muted (buzzer OFF) and does not create a false positive when enabling the buzzer.
