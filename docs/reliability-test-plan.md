...
4. **Buzzer legacy chain**: In legacy mode, if `set_buzzer_volume` fails (command error), the transport retries multiple known firmware variants: `set_buzzer` (`"on"/"off"`, boolean, numeric, and no-arg), then `set_sound` variants, then `set_mute` variants (inverse semantics) for `zhimi.airpurifier.pro` compatibility branches.
