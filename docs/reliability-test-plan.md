...
8. **Post-write state verification on Pro**: Some Pro firmware variants acknowledge a buzzer setter call but do not actually change the state. After each successful legacy buzzer write attempt, the plugin immediately probes buzzer aliases (`get_prop`) and only exits when observed state matches requested on/off target; otherwise it continues fallback variants.
