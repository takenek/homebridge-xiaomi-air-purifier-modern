...
4. **Buzzer legacy chain**: In legacy mode, if `set_buzzer_volume` fails (command error), the transport retries `set_buzzer` with `"on"`/`"off"`, then boolean, then numeric payloads (`1`/`0`) to cover firmware variants on `zhimi.airpurifier.pro`.
